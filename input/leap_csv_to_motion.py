...
coarse post-shaking consolidation tail. This script:
  ...
  3. builds the base-input motion (per the LEAP spec, the **average of AH11 and
     AH12**, the two container-base accelerometers) on a uniform dt,
  4. writes ``input/base_motion.txt``  (time[s]  accel[g])  -> used by the model,
  5. writes ``data/leap_recorded.csv`` (trimmed, uniform) -> used by
     ``postprocess/compare_with_leap.py`` to overlay the measured response.

LEAP-UCD-2017 base motion (Kutter et al., 2017/2018)
----------------------------------------------------
"The input base motion is taken as the average of the accelerations measured by
AH11 and AH12." Hence the default base = mean(AH11, AH12). Always confirm the
channel names against the sensor-layout sheet for your specific test.

Usage
-----
    python input/leap_csv_to_motion.py --list
    python input/leap_csv_to_motion.py --base "AH11 (g),AH12 (g)" --t0 0 --t1 25
"""
...
# LEAP input base motion = average of the two container-base accelerometers
BASE_CHANNELS = ["AH11 (g)", "AH12 (g)"]   # VERIFY vs sensor-layout sheet!
...
def convert(path, base_channels, t0, t1):
    header, data = read_csv(path)
    missing = [c for c in base_channels if c not in header]
    if missing:
        raise SystemExit(
            f"channel(s) {missing} not found. Available:\n  "
            + "\n  ".join(header))
    ...
    # base = average of the requested container-base channels
    idx = [header.index(c) for c in base_channels]
    base = np.mean([np.interp(uni_t, tt, data[keep, i]) for i in idx], axis=0)
    base = base - base[0]                              # remove DC offset

    with open(MOTION_FILE, "w") as f:
        f.write(f"# LEAP base motion from {os.path.basename(path)} "
                f"= mean({', '.join(base_channels)}), prototype scale\n")
        f.write("# col1: time [s]   col2: acceleration [g]\n")
        for ti, ai in zip(uni_t - uni_t[0], base):
            f.write(f"{ti:.5f}\t{ai:.6f}\n")
    print(f"wrote {MOTION_FILE}  (base = mean of {base_channels})")
    print(f"  {base.size} samples, dt={target_dt:.4f}s, "
...
def main():
    ...
    ap.add_argument("--base", default=",".join(BASE_CHANNELS),
                    help="comma-separated base channel(s); averaged if >1")
    ...
    if args.list:
        ...
        print("\nBase motion defaults to mean(AH11, AH12) per the LEAP spec; "
              "override with --base \"AHx (g),AHy (g)\".")
        return

    base_channels = [c.strip() for c in args.base.split(",") if c.strip()]
    convert(args.csv, base_channels, args.t0, args.t1)
